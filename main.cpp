...
VOID SetSkin(HWND hwnd, CONST CHAR sz_skin[])
{
	...
	const int operationButtons[] = { IDC_BUTTON_POINT, IDC_BUTTON_SLASH, IDC_BUTTON_ASTER, IDC_BUTTON_MINUS, 
		IDC_BUTTON_PLUS, IDC_BUTTON_BSP, IDC_BUTTON_CLR, IDC_BUTTON_EQUAL  };
	const char* operationNames[] = { "point", "slash", "aster", "minus", "plus", "bsp", "clr", "equal" };

	for (size_t i = 0; i < sizeof(operationButtons) / sizeof(operationButtons[0]); i++)
	{	
		sprintf(sz_filename, "BMP\\%s\\button_%s.bmp", sz_skin, operationNames[i]);
		HBITMAP bmpIcon = (HBITMAP)LoadImage
		(
			GetModuleHandle(NULL),
			sz_filename,
		...
		//PrintLastError(GetLastError());
		//MessageBox(hwnd, lpszMessage, "", MB_OK);
		SendMessage(GetDlgItem(hwnd, operationButtons[i]), BM_SETIMAGE, IMAGE_BITMAP, (LPARAM)bmpIcon);
		hButtonBitmaps[i + 10] = bmpIcon;
	}	
